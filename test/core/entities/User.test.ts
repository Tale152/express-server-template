...
test('Username and password should not contain extra spaces', () => {
  const user: User = User.createInstance(addExtraSpaces(validUsername), addExtraSpaces(validPassword));
  expect(user.username).toEqual(validUsername);
  expect(user.password).toEqual(validPassword);
})

function assertUserThrowsOnNewInstance(username: string, password: string): void{
